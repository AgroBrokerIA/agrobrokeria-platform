...
function ivaId(rate:number|null){const r=Number(rate||0);if(r===21)return 5;if(r===10.5)return 4;if(r===27)return 6;if(r===5)return 8;if(r===2.5)return 9;if(r===0)return 3;throw new Error("IVA_RATE_NOT_SUPPORTED")}
function makeCms(){
...
async function getToken(){const cms=makeCms();const body='<?xml version="1.0"?><soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/"><soapenv:Body><loginCms xmlns="http://wsaa.afip.gov.ar/ws/services/LoginCms"><in0>'+cms+'</in0></loginCms></soapenv:Body></soapenv:Envelope>';const r=await fetch(WSAA,{method:"POST",headers:{"content-type":"text/xml; charset=utf-8"},body});const xml=await r.text();if(!r.ok)throw new Error("ARCA_WSAA_HTTP_"+r.status);const token=nodeText(xml,"token"),sign=nodeText(xml,"sign");if(!token||!sign)throw new Error("ARCA_WSAA_INVALID_RESPONSE");return{token,sign}}
async function soap(method:string,inner:string,a:{token:string,sign:string}){const body='<?xml version="1.0"?><soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/"><soapenv:Body><'+method+' xmlns="http://ar.gov.afip.dif.FEV1/"><Auth><Token>'+esc(a.token)+'</Token><Sign>'+esc(a.sign)+'</Sign><Cuit>'+esc(Deno.env.get("ARCA_CUIT")||"")+'</Cuit></Auth>'+inner+'</'+method+'></soapenv:Body></soapenv:Envelope>';const r=await fetch(WSFE,{method:"POST",headers:{"content-type":"text/xml; charset=utf-8","SOAPAction":'"http://ar.gov.afip.dif.FEV1/'+method+'"'},body});return{ok:r.ok,status:r.status,xml:await r.text()}}
async function authorize(i:any){
 const a=await getToken();
 const last=await soap("FECompUltimoAutorizado","<PtoVta>"+i.punto_venta+"</PtoVta><CbteTipo>"+i.tipo_comprobante_codigo+"</CbteTipo>",a);
 const next=Number(nodeText(last.xml,"CbteNro")||nodeText(last.xml,"CbteDesde")||"0")+1;
 const date=String(i.fecha_emision).replace(/-/g,""),total=Number(i.importe_total).toFixed(2),net=Number(i.importe_neto).toFixed(2),iva=Number(i.importe_iva||0).toFixed(2);
 const mon=String(i.moneda_codigo||"").toUpperCase();if(!["PES","DOL","EUR"].includes(mon))throw new Error("CURRENCY_NOT_CONFIGURED");
 const ivaXml=Number(i.importe_iva||0)>0?"<Iva><AlicIva><Id>"+ivaId(i.iva_alicuota)+"</Id><BaseImp>"+net+"</BaseImp><Importe>"+iva+"</Importe></AlicIva></Iva>":"";
...
Deno.serve(async req=>{
 try{
  ...
  try{
   const enriched={...i,moneda_codigo:i.monedas?.codigo};const o=await authorize(enriched),ok=o.result==="A"&&!!o.cae,v=o.venc&&/^\d{8}$/.test(o.venc)?o.venc.slice(0,4)+"-"+o.venc.slice(4,6)+"-"+o.venc.slice(6,8):null;
   await db.from("facturas").update({estado:ok?"AUTORIZADA":o.result==="O"?"OBSERVADA":"RECHAZADA",numero_comprobante:o.cbte,arca_numero_solicitado:o.cbte,cae:o.cae,cae_vencimiento:v,arca_resultado:o.result,arca_codigo:o.codes.join(","),arca_mensaje:o.messages.join(" | "),arca_request_xml:o.request,arca_response_xml:o.response,arca_autorizado_at:ok?new Date().toISOString():null,actualizada_en:new Date().toISOString()}).eq("id",id);
   await db.from("factura_eventos").insert({factura_id:id,actor_profile_id:user.id,evento:ok?"ARCA_AUTORIZADA":"ARCA_RECHAZADA",metadata:{resultado:o.result,codes:o.codes,messages:o.messages,cae:o.cae,comprobante:o.cbte}});
