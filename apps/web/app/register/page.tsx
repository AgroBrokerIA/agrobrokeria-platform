...
export default function RegisterPage() {
  const [form, setForm] = useState({ nombre:"", empresa:"", cuit:"", telefono:"", provincia:"", tipoUsuario:"comprador", email:"", password:"" });
  const [loading, setLoading] = useState(false);
  function update(field: keyof typeof form, value: string) { setForm((current) => ({ ...current, [field]: value })); }

  async function registrarse() {
    if (!form.nombre.trim() || !form.email.trim() || form.password.length < 8) { alert("Completá nombre, correo y una contraseña de al menos 8 caracteres."); return; }
    setLoading(true);
      ...
      <section className="auth-panel">
        <div className="auth-card auth-register-card">
          ...
          <form onSubmit={(e) => { e.preventDefault(); void registrarse(); }}>
            ...
            <label>Contraseña<input type="password" placeholder="Mínimo 8 caracteres" value={form.password} onChange={(e)=>update("password",e.target.value)} minLength={8} required /></label>
