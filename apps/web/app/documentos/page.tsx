...
export default function DocumentosPage(){
 ...
 const [loading,setLoading]=useState(true),[error,setError]=useState(""),[generando,setGenerando]=useState<"LOI"|"SCO"|null>(null),[mensaje,setMensaje]=useState("");

 useEffect(()=>{(async()=>{
   try{
     const {data:{user}}=await supabase.auth.getUser();
     if(!user) throw new Error("Necesitás iniciar sesión.");
     const [{data:d,error:de},{data:c,error:ce},{data:o,error:oe},{data:l,error:le},{data:s,error:se}]=await Promise.all([
     ...
     if(de) throw new Error(de.message); if(ce) throw new Error(ce.message); if(oe) throw new Error(oe.message); if(le) throw new Error(le.message); if(se) throw new Error(se.message);
     setDocs((d||[]) as Documento[]); setContratos((c||[]) as Contrato[]); setOperaciones((o||[]) as Operacion[]); setLois((l||[]) as Comercial[]); setScos((s||[]) as Comercial[]);
 ...
 async function generarComercial(tipo:"LOI"|"SCO"){
   ...
   try{
     ...
     const {data,error}=await supabase.rpc(tipo==="LOI"?"crear_loi_comercial":"crear_sco_comercial",{p_operacion_id:op.id,p_archivo_pdf:dataUri});
     ...
     setLois((l||[]) as Comercial[]); setScos((s||[]) as Comercial[]);
 ...
 return <main className="module-page documents-page">
   <div className="module-hero"><div><span className="eyebrow">EXPEDIENTE</span><h1>Documentos</h1><p>Expedientes de empresa, contratos y documentación vinculada a operaciones.</p></div><div className="module-pill">{docs.length+contratos.length+empresaDocs.length+lois.length+scos.length} registros</div></div>
...
{firmaContrato&&<div style={{padding:12,background:"#f1f5f9",borderRadius:8,wordBreak:"break-all"}}><b>Enlace de firma:</b><br/>{firmaContrato}<br/><button type="button" onClick={()=>navigator.clipboard?.writeText(firmaContrato)}>Copiar enlace</button></div>}
     ...
     <section className="document-section"><div className="document-section-head"><div><span className="eyebrow">EMISIONES COMERCIALES</span><h2>Historial LOI / SCO</h2></div></div>
       {lois.length===0&&scos.length===0?<div className="module-empty compact"><p>Todavía no hay LOI o SCO emitidos.</p></div>:<div className="document-grid">
