  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase.from("profiles")
        .select("nombre,empresa,tipo_usuario").eq("id", auth.user.id).maybeSingle();
      setProfile(data);
      setLoading(false);
    }
    void load();
  ...
  return (
    <main style={{ maxWidth: 1200, margin: "0 auto", padding: 24 }}>
      ...
      <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(230px,1fr))", gap: 16 }}>
        <Link href="/marketplace" style={card}><strong>🌾 Marketplace</strong><span>Consultá publicaciones agrocomerciales disponibles.</span></Link>
...
const card = {
  textDecoration: "none", color: "#0f172a", padding: 20, borderRadius: 14,
  background: "#fff", border: "1px solid #e2e8f0", display: "grid", gap: 8,
};
